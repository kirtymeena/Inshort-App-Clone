/* eslint-disable react/prop-types */
import "./NewsContent.css"
import { Container } from "@mui/material"
import NewsCard from "../NewsCard/NewsCard"
import { Button } from "@mui/material"

function NewsContent({ newsResults, limit, totalContent, setLimit }) {
    console.log("result", newsResults)
    return (
        <Container maxWidth='md'>
            <div className="downloadMsg">

                <div>
                    For the best experience use <span style={{ fontWeight: "500" }}>inshort</span> app on your smartphone.
                </div>
                <div className="playstore__icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" id="google-play"><path fill="#2196F3" d="M8.32 7.68.58 15.42c-.37-.35-.57-.83-.57-1.35V1.93C.01 1.4.22.92.6.56l7.72 7.12z"></path><path fill="#FFC107" d="M15.01 8c0 .7-.38 1.32-1.01 1.67l-2.2 1.22-2.73-2.52-.75-.69 2.89-2.89L14 6.33c.63.35 1.01.97 1.01 1.67z"></path><path fill="#4CAF50" d="M8.32 7.68.6.56C.7.46.83.37.96.29 1.59-.09 2.35-.1 3 .26l8.21 4.53-2.89 2.89z"></path><path fill="#F44336" d="M11.8 10.89 3 15.74c-.31.18-.66.26-1 .26-.36 0-.72-.09-1.04-.29a1.82 1.82 0 0 1-.38-.29l7.74-7.74.75.69 2.73 2.52z"></path></svg>
                    <small className="icon__text">Android App on <span style={{ fontSize: "16px", fontWeight: 500 }}>Google Play</span></small>
                </div>


            </div>
            <div className="news__stack">
                {
                    newsResults && newsResults.map(news =>
                        <>
                            <NewsCard news={news} />
                        </>
                    )

                }

            </div>
            <div className="load__btn">
                {
                    limit < totalContent &&
                    <Button disabled={newsResults.length < limit} variant="contained" sx={{ backgroundColor: "white", color: "black" }} onClick={() => setLimit(prev => prev + 30)}>Load More</Button>
                }
            </div>
        </Container>
    )
}

export default NewsContent